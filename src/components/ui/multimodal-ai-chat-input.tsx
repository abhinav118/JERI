...
// Button variants using cva - adapted for dark theme
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        default: 'bg-browser-accent text-browser-bg hover:bg-browser-accent/80',
        destructive: 'border border-browser-red text-browser-red hover:bg-browser-red/10',
        outline: 'border border-browser-surface bg-transparent hover:bg-browser-surface hover:text-browser-text',
        secondary: 'bg-browser-surface text-browser-text hover:bg-browser-surface/80',
        ghost: 'text-browser-text hover:bg-browser-surface hover:text-browser-text',
        link: 'text-browser-accent underline-offset-4 hover:underline',
      },
      size: {
...
>(({ className, ...props }, ref) => {
  return (
    <textarea
      className={cn(
        'flex min-h-[80px] w-full rounded-md border border-browser-surface bg-browser-bg px-3 py-2 text-base ring-offset-browser-bg placeholder:text-browser-subtext focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-browser-accent focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm text-browser-text',
        className,
      )}
      ref={ref}
      {...props}
...
const PreviewAttachment = ({
  ...
  return (
    <div data-testid="input-attachment-preview" className="flex flex-col gap-1">
      <div className="w-20 h-16 aspect-video bg-browser-surface rounded-md relative flex flex-col items-center justify-center overflow-hidden border border-browser-surface">
        {contentType?.startsWith('image/') && url ? (
          ...
          <div className="flex items-center justify-center text-xs text-browser-subtext text-center p-1">
        ...
        {isUploading && (
          <div
            ...
            className="animate-spin absolute text-browser-accent"
          >
            <LoaderIcon className="size-5" />
          </div>
        )}
      </div>
      <div className="text-xs text-browser-subtext max-w-20 truncate">
        {name}
      </div>
    </div>
  );
...
function PureAttachmentsButton({
  ...
  return (
    <Button
      ...
      className="rounded-md rounded-bl-lg p-[7px] h-fit border border-browser-surface hover:bg-browser-surface"
      onClick={(event) => {
...
function PureStopButton({ onStop }: { onStop: () => void }) {
  return (
    <Button
      ...
      className="rounded-full p-1.5 h-fit border border-browser-red text-browser-red hover:bg-browser-red/10"
      onClick={(event) => {
        event.preventDefault();
        onStop();
      }}
  ...
  const handleFileChange = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (invalidFiles.length > 0) {
        console.warn(`Skipped ${invalidFiles.length} files larger than 25MB.`);
    ...
    <div className={cn("relative w-full flex flex-col gap-2", className)}>
      ...
      {(attachments.length > 0 || uploadQueue.length > 0) && (
        <div
          ...
          className="flex flex-row gap-2 overflow-x-auto items-end pb-1"
        >
          {attachments.map((attachment) => (
            <div key={attachment.url || attachment.name} className="relative group">
              ...
              <Button
                variant="destructive"
                size="icon"
                className="absolute top-[-6px] right-[-6px] h-4 w-4 rounded-full p-0 flex items-center justify-center z-20 opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      <Textarea
        ...
        className={cn(
          'min-h-[24px] max-h-[120px] overflow-y-auto resize-none rounded-xl !text-sm pb-8 pr-10',
          'bg-browser-surface border border-browser-surface/50',
        )}
        rows={1}
        autoFocus
      ...
      <div className="absolute bottom-0 left-0 p-1.5 w-fit flex flex-row justify-start z-10">
        <AttachmentsButton
      ...
      <div className="absolute bottom-0 right-0 p-1.5 w-fit flex flex-row justify-end z-10">
        {isGenerating ? (
...
export type { Attachment, UIMessage, MultimodalInputProps };
